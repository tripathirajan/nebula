...
**Naming note:** `headless` here is the *renamed* version of what this repo was calling `@nebula-lab/styleless` (itself renamed from the original `@nebula-lab/headless` — see task #17). **This rename is now done**: `packages/styleless` → `packages/headless` landed (package name, imports across the repo, tsconfig/eslint path references, story titles, docs). `styleless` is a genuinely **new** layer being introduced — it did not exist before this taxonomy and has not been built yet. This exact 5-tier model was independently reconfirmed by the project owner via a 4-question decision guide (Primitive → Headless → Styleless → React UI), now embedded verbatim in `AGENTS.md`'s "Layer placement" section — treat this taxonomy as locked, not tentative.
...
6. **`Fragment` is not a component to build — it's `React.Fragment`.** Not listed below. **`Polymorphic` is a type (`PolymorphicComponent<E, P>`), not a runtime component** — already exists in `@nebula-lab/primitives/types`; not listed below as a buildable item.
...
### Data
`Table`/`DataGrid` **[extracted]** (today `react-ui`'s `DataTable`/`DataGrid`) — both had real structure worth decoupling (`DataTable`: `useControllableState` ×3 for sort/selection state, a `DataTableContext`, a real sort-toggle `<button>` computing `aria-sort`, checkbox selection with indeterminate "some selected" math; `DataGrid`: `@nebula-lab/hooks`' `useVirtualizer`, the WAI-ARIA Grid pattern built from scratch on plain `div`s, absolute-position math) — these were two structurally different pieces (a stateful table vs. a self-contained virtualized grid) bundled under one taxonomy bullet, built as two separate `styleless` components, not one. Both now live in `@nebula-lab/styleless` (`data-table`, `data-grid`), with `react-ui` reduced to thin themed wrappers supplying `classNames`. `DataGrid`'s absolute-position/transform math (functionally required for virtualization, not a visual choice) is baked into the `styleless` layer's own inline style; `display: flex` on the row/header/cell parts is left to `react-ui`'s `classNames` since it's a decorative layout choice, not a structural requirement. · `DescriptionList`/`List`/`ListItem` **[resolved: skip `styleless`]** — inspected: both are plain `Primitive` + `cn()` wrappers (`List`'s only extra prop, `ordered`, just picks `ol` vs `ul`), no state, no headless compound to decouple from — same "already at simplest form" conclusion as the Layout row above
...
`LoginForm` **[built, react-ui-blocks]** · `RegisterForm`/`SignupForm` **[new, react-ui-blocks]** · `PasswordStrengthIndicator` **[built]** — legitimately `styleless`-tier (a themed meter, no page-level composition), not a block; the real logic (a length/character-class scoring heuristic, exported standalone as `getPasswordStrength` so a form's own validation can reuse the same score) lives in `@nebula-lab/styleless`, `role="meter"` with a required accessible name (`aria-label`, defaulting to `"Password strength"` — axe's `aria-meter-name` rule flags a meter with none). `react-ui`'s themed version colors each segment red/yellow/green by score tier via `group`/`group-data-[score=N]` off the track's own `data-score`, and gives each segment a `min-w-6` floor alongside `flex-1` so it still renders visibly under an unconstrained-width ancestor (a real bug caught live in Storybook: `flex-basis: 0%` items collapse to ~0px when nothing up the tree provides a definite width) — new `passwordStrengthTokens` (`track`, `labelText`) added to `tokens/component.ts`.
