...
| `packages/styleless` | Stable, coverage-complete per `LAYER_TAXONOMY.md` §4's audit. ~18 components extracted (`button`, `icon-button`, `split-button`, `fab`, `input`, `search-input`, `password-input`, `email-input`/`tel-input`/`url-input`, `textarea`, `avatar`/`avatar-group`, `image-preview`, `data-table`, `data-grid`, `code-block`, `password-strength-indicator`), each wired into a `react-ui` consumer. Everything else in the original ~50-item extraction plan was investigated and explicitly resolved as either a duplicate or having no real behavior left to decouple from `headless`/`react-ui` (see §4's per-item `[resolved: skip]` notes); a handful (`CloseButton`, `MaskInput`, `CurrencyInput`, `NavigationRail`, `LocaleSwitcher`) remain `[new, deferred]` pending a real consumer. See `packages/styleless/README.md`. |
...
| `templates/dashboard-ui`, `templates/mobile-ui` | Example CSR apps (private, unpublished workspace packages, not part of the `@nebula-lab/*` release pipeline) — real consumers of `react-ui`/`react-ui-blocks` via the pnpm-workspace `workspace:*` protocol, not Storybook stories. Each has its own `README.md` with run instructions and the Tailwind `@source` gotcha every out-of-repo consumer will also hit. |
