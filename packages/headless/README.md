...
Behavior-only, ARIA-complete compound components. No styling opinions — that's `@nebula/react-ui`'s job (via the new, not-yet-built `@nebula/styleless` middle layer, once it exists — see `LAYER_TAXONOMY.md`), built on top of this package. (This package was renamed `@nebula/headless` → `@nebula/styleless` → back to `@nebula/headless`: the project owner confirmed a 5-tier model where `headless` — this package — and `styleless` — a distinct, new unstyled-but-structurally-complete layer — are two different things, not one renamed package. See `AGENTS.md`'s "Layer placement" section.)
...
```tsx
import { Tabs, TabList, Tab, TabPanel } from '@nebula/headless';
// or per-component subpath
import { Tabs, TabList, Tab, TabPanel } from '@nebula/headless/tabs';
import { Checkbox } from '@nebula/headless/checkbox';
import { Switch } from '@nebula/headless/switch';
import { RadioGroup, RadioGroupItem } from '@nebula/headless/radio-group';
import {
  Accordion,
  AccordionItem,
  AccordionHeader,
  AccordionTrigger,
  AccordionContent,
} from '@nebula/headless/accordion';
import {
  Dialog,
  DialogTrigger,
  DialogPortal,
