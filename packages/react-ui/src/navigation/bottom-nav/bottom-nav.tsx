 ...
 * iOS home-indicator or Android gesture bar. `--z-bottom-nav` is
 * deliberately below `--z-overlay` (every floating overlay in this package —
 * `Dialog`/`Drawer`/`Toast`/etc. — see `tokens/primitive.ts`'s `zIndex`
 * group), so those always stack above the tab bar.
 *
...
const BottomNav = React.forwardRef<HTMLElement, BottomNavProps>((props, forwardedRef) => {
  ...
  return (
    <Primitive
      as="nav"
      className={cn(
        'fixed inset-x-0 bottom-0 z-[var(--z-bottom-nav)] flex items-stretch justify-around border-t bg-(--bottom-nav-bg) text-(--bottom-nav-item-text) border-(--bottom-nav-border) pb-[env(safe-area-inset-bottom)] md:hidden',
        className,
      )}
      role='navigation'
      {...rest}
