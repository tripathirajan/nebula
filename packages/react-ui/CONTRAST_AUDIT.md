...
Generated by `pnpm --filter @nebula-lab/react-ui contrast-audit` (`src/tokens/contrast-audit.ts`), which computes WCAG relative luminance + contrast ratio for every meaningful semantic color pairing. Re-run this after touching `primitive.ts` or `semantic.ts` — nothing enforces it automatically yet (see `STATUS_AND_COMPLIANCE_REPORT.md` §"Automated accessibility testing").
...
## Focus-ring color audit, this pass

A repo-wide focus-ring harmonization sweep re-checked every component's `focus-visible:ring-[var(...)]` color choice against these same thresholds (3:1, since a ring is a UI/non-text boundary), not just the text pairings above.

- **Found and fixed a real failure**: `AlertDialogAction` rang on `--button-danger-bg` (raw `--color-error`) — 2.92:1 against `base.100` in light mode, under the 3:1 minimum by a hair. Fixed to ring `--color-base-content` instead, matching both `Button`'s own uniform-ring convention and its sibling `AlertDialogCancel` (already correctly on `--dialog-text` = `--color-base-content`).
- **Corrected stale documentation, not a code bug**: `button.tsx`'s and `contrast-audit.ts`'s own comments both claimed `primary` "only clears ~2:1" against `base.100`, citing a since-changed "76% lightness" — recomputed directly, `primary` vs `base.100` is actually **14.82:1** (light) / **3.97:1** (dark), comfortably passing. The comments were written against an earlier version of the palette, before `primary` was finalized to today's dark-navy value, and never revisited. `Button` still correctly rings every color in `base.content` regardless — not because `primary` itself needs it, but because it keeps one ring color safe and predictable across every `color` value, several of which (`error`/`success`/`warning`) really do fail on their own, per the entries above.
- **Everything else checked out**: every "checked/active/selected" component that rings its own state color (`Checkbox`, `Switch`, `RadioGroup`, `PaginationLink`, `Stepper`, `Carousel` indicators/arrows, `Tree`, `Tabs`, `NavigationMenu`, `Calendar`) rings `--color-primary`, which — per the recomputed 14.82:1 / 3.97:1 above — is safe in both themes. No further changes needed there.
