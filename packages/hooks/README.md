...
React hooks with no dependency on `@nebula/primitives` or any higher layer — state, DOM, and lifecycle utilities every other package can build on.
...
```ts
import { useControllableState, useBoolean } from '@nebula/hooks';
// or per-hook subpath
import { useControllableState } from '@nebula/hooks/use-controllable-state';
```

## Note on `@nebula/utilities`

This package intentionally does not import from `@nebula/utilities` (see `component-library-architecture.md` §2: both packages sit at the bottom of the dependency graph with no cross-dependency). Where a hook needs logic that overlaps with a utility (e.g. `useFocusTrap`'s focusable-element query vs. `isFocusable`), it's duplicated locally with a comment pointing at the utilities equivalent rather than creating a cross-import.
