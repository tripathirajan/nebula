...
import { useStableCallback } from '../use-stable-callback/use-stable-callback';
      ...
      if (isControlled) {
        const resolved =
          typeof nextValue === 'function'
            ? (nextValue as (current: T) => T)(prop as T)
            : nextValue;
        if (resolved !== prop) handleChange(resolved);
      } else {
        setUncontrolledProp((current) => {
          const resolved =
            typeof nextValue === 'function'
              ? (nextValue as (currentValue: T) => T)(current as T)
              : nextValue;
          if (resolved !== current) handleChange(resolved);
          return resolved;
        });
      }
    },
 ...
 * ```
 */
function useIsControlledWarning(isControlled: boolean, componentName: string) {
  const wasControlledRef = useRef(isControlled);
  if (wasControlledRef.current !== isControlled && process.env.NODE_ENV !== 'production') {
     
    console.warn(
      `${componentName} is changing from ${wasControlledRef.current ? '' : 'un'}controlled to ` +
        `${isControlled ? '' : 'un'}controlled. Components should not switch between controlled ` +
        'and uncontrolled across renders.',
    );
  }
