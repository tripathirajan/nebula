...
| 9 | Full primitives feature set: Layout (`Box`/`Flex`/`Grid`/`Stack`/`Inline`/`Center`/`Container`/`AspectRatio`), Text (`Text`/`Heading`/`Paragraph`/`Code`/`Pre`/`Link`), Accessibility (`VisuallyHidden`/`FocusScope`/`DismissableLayer`/`Boundary`/`RovingFocusGroup`+`FocusItem`), Form (`Button`/`Input`/`Textarea`/`Label`/`Form`), Visibility (`Portal`/`Presence`/`Overlay`) | `packages/primitives/src/*` | See Part 2 for how each holds up against WAI-ARIA. |
