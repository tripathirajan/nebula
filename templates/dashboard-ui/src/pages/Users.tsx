    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <Text className="mt-1 opacity-70">Manage your team members and their account permissions here.</Text>
        </div>
      ...
      <DataTableBlock
        columns={[
          ...
          { key: 'role', header: 'Role', render: (user) => user.role },
          {
            ...
            header: 'Status',
            render: (user) => (
              <Badge color={statusColor[user.status]}>{user.status.charAt(0).toUpperCase() + user.status.slice(1)}</Badge>
            ),
          },
        ]}
        rows={filtered}
        ...
        renderCard={(user) => (
          <CardListItem
            ...
            trailing={<Badge color={statusColor[user.status]}>{user.status.charAt(0).toUpperCase() + user.status.slice(1)}</Badge>}
            actions={
              <>
                <MenuItem onSelect={() => {}}>Edit</MenuItem>
                <MenuItem onSelect={() => {}}>Delete</MenuItem>
