...
One table per package — every component/primitive currently in the monorepo, what it does, and how it relates to the layer above/below it (per \`ARCHITECTURE.md\`'s layering: \`primitives\` → \`headless\` → \`styleless\` → \`react-ui\` → \`react-ui-blocks\`).
