...
- [ ] Virtualizer — N/A in `headless`/`react-ui` (project owner decision): pure windowing math with no ARIA semantics of its own — **built this session** as `useVirtualizer` in `@nebula/hooks`, not a component in either layer
...
- [x] Header — built this session (purely presentational, no `@nebula/headless` compound underneath, same "thin `cn()` wrapper around `Primitive`" treatment as `Card`)
...
- [x] PasswordField — built this session (a genuinely new `react-ui`-layer component, no `@nebula/headless` counterpart — a built-in show/hide toggle that swaps the underlying native `<input>`'s `type` between `"password"`/`"text"` rather than hiding text via CSS, so autofill/password managers keep working correctly either way)
...
- [x] TextArea — built this session (wraps `@nebula/primitives`' unstyled `Textarea` the same way this package's `Input` wraps `@nebula/primitives`' `Input` — there's no `@nebula/headless` layer for it either, mirroring how `headless` itself has no `Textarea`)
